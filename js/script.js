document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        socket.on('protopie', function (data) {
            ...
            if (data.messageId === "Neustart" || data.messageId === "restart" || data.messageId === "reset") {
                console.log('🔄 [Socket.IO] Neustart empfangen - Seite wird neu geladen');

                // ROBUSTER Reload – funktioniert auch bei manchen Blockierungen
                setTimeout(() => {
                    console.log('⏳ Seite wird jetzt wirklich neu geladen...');
        ...
        while (!pathComplete && segmentCount < maxSegments) {
            ...
            if (nextIntersection) {
                if (nextIntersection.type === 'prism') {
                    ...
                    // Nicht reflektieren – gehe direkt zum nächsten Schleifendurchlauf
                    segmentCount++;
                    continue;
                }

                if (nextIntersection.type === 'mirror') {
                    drawLaserSegment(start, nextIntersection.point);
